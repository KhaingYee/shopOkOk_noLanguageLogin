...
<!--            :src="URL + $store.state.commodity_data.images[0].pic_url"-->
            ...
            <p v-else class="item" style="padding-top: 0.2rem;">{{data.goods.title }}{{activeItem}}</p>
<!-- data.goods.title -->
            <span v-if="tempequal == false" class='item' style='color:red;font-size:14px'>
              ...
              <em >{{$store.state.commodity_data.goods.panic_price}}</em>
            </span>
            <span v-else class='item' style='color:red;font-size:14px'>
              ￥
              <em v-if="$store.state.commodity_data.goods.p_id==0">{{productData.panic_price}}</em>
              <em v-else-if="$store.state.commodity_data.goods.panic_price">{{$store.state.commodity_data.goods.panic_price}}</em>
              <em v-else>{{$store.state.commodity_data.goods.panic_price}}</em>
            </span>
            ...
            <p class="price" v-else>
              ...
              <!-- <span v-else>{{$store.state.commodity_data.goods.panic_price}}积分</span> integral product mha 积分 paw ya mr -->
              <span
                v-if="$store.state.commodity_data.goods.packaging"
              >{{$store.state.commodity_data.goods.packaging}}</span>
              <span v-else></span>
          ...
          <div class="content-scroll">
              ...
              >
                ...
                <div v-if="tempequal == false">
                    <div v-for="(data, i) in spec.spec_children" :key="'A' + i">
                        <span v-if="item.id == data.spec_id"
                            ...
                            @click="addClassEdit(index,$event,data.id, item.id); sendValueEdit(data, data.id, subIndex[index])"
                        >
                            {{data.item}}
                        </span>
                    </div>
                ...
                <div v-else>
                    <div v-for="data in spec.spec_children" :key="data.id">
                        ...
                        <span v-if="(item.id == data.spec_id) && !(assemPro)"
                              :data-id="data.id"
                              :data-spec-id="data.spec_id"
                              :class="{ gray: emptyStockGoods.indexOf(Number(data.id)) != -1, activeTab: activeItem == data.item }"
                              @click="addClassTrue(index,$event,data.id, item.id); sendValueTrue(data, data.id, subIndex[index])"
                        >
                            {{data.item}}
                        </span>
                        <span v-if="(item.id == data.spec_id) && (assemPro && assemPro=='assemPro')"
                              :data-id="data.id"
                              :data-spec-id="data.spec_id"
                              :class="{ gray: emptyStockGoods.indexOf(Number(data.id)) != -1, activeTab: activeItem == data.item }"
                              @click="addClassAP(index, data.id, data)"
                        >
                            {{data.item}}
                        </span>
                    </div>
                ...
                <div class="pull-right fr clearfix">
                    ...
                    <div class="input-main fl clearfix">
                        ...
                        <input
                            type="tel"
                            class="fl"
                            :maxlength="data.goods.goods_count.length"
            ...
            <div class="com">
              ...
              <div>
                ...
                <span v-else>
                  <i>￥</i>
                  {{$store.state.commodity_val * $store.state.commodity_data.goods.panic_price}}
                </span>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    sendValueTrue(detail, id, subId){
        this.selected = detail;
        this.detailId = id
        this.detailSubId = subId;
        if(this.activeItem == '') {
            if(this.nospec == '') {
                this.$emit('sendValue', null, null, null);
            } else {
                this.$emit('sendValue', null, null, this.nospec);
            }
        } else {
    ...
    sendValueEdit(detail, id, subId){
        let count = 0;
        for(let i = 0; i < this.spec.spec_children.length; i++) {
            if(this.spec.spec_children[i].selectedItem && this.spec.spec_children[i].selectedItem == true) {  
                count++;
            }
        }
        if(count > 0) {
            let option = ''
            let optArr = []
            for(let j = 0; j < this.spec.spec_children.length; j++) {
                if(this.spec.spec_children[j].selectedItem && this.spec.spec_children[j].selectedItem == true) {
                    option += this.spec.spec_children[j].item
                    option += ", "
                    optArr.push(this.spec.spec_children[j].item)
                }
            }
            this.temp1 = option
            this.temp2 = optArr
            this.$emit('sendValue', option, null, null, optArr);
        } else {
            this.selected = detail;
            this.detailSubId = subId;
            this.detailId = id
            if(this.activeItem == '') {
                if(this.nospec == '') {
                    this.$emit('sendValue', null, null, null, null);
                } else {
                    this.$emit('sendValue', null, null, this.nospec, null);
                }
            } else {
                this.$emit('sendValue', detail, id, subId, 'speciData');
            }
        }
        this.arrayKey = id;
        // this.zeroPrice();
    ...
    getPackageSub(id){
      ...
      if(id){
        ...
        _id = this.$store.state.commodity_data.goods.id
      }
      this.axios.post(
...
if(this.tempequal == false && this.spec.spec_children[0].selectedItem == true) {
  ...
  for (var j = 0; j < this.spec.spec_children.length; j++) {
      this.childitems += this.spec.spec_children[j].item+ " "
            ...
            if(this.assemPro == 'assemPro') {
                if (res.data.status == 1) {
                    ...
                    for (let i in temp1) { 
                        if(temp1[i].id == this.data.goods.id) {
            ...
            if (
              ...
              for (var i in this.guigedata.goods) {
                if (this.guigedata.goods[i].goods_count == 0) {
    ...
    plus() {
      if (
        this.$store.state.commodity_val >=
        parseInt(this.$store.state.commodity_data.goods.goods_count)
      )
        return;
      this.$store.state.commodity_val++;
    },
    ...
    min() {
      ...
      if (val >= this.data.goods.goods_count)
        this.$store.state.commodity_val = this.data.goods.goods_count;
    },
    minRice() {
      var match = /^(\d+(\.\d)?)(\d*)$/;
    ...
    addClassAP(index, id, detail) {
        ...
        if(this.tempequal == false) {
          for(let i = 0; i < this.spec.spec_children.length; i++) {
              if(this.spec.spec_children[i].spec_id == id) {
        ...
        if(this.getArrayLenght(this.subIndex) == this.getArrayLenght(this.spec.spec_group)) {
            //选中商品后重新选择规格 清除选中的规格项
            this.$store.state.selectionSpec = [];
        }

        if(this.getArrayLenght(this.subIndex) == this.getArrayLenght(this.spec.spec_group) && this.getArrayLenght(this.subIndex) > 2) {
            this.subIndex = [];
        ...
        setTimeout(function() {
            ...
            if(self.tempequal == false && Object.keys(selectedList).length != self.spec.spec_children.length) {
                self.getData(self.$route.params.goods_id);
        ...
        if(this.tempequal == true && this.assemPro && (this.assemPro == 'assemPro')) {
            for(let i = 0; i < this.spec.spec_children.length; i++) {
                if(this.spec.spec_children[i].id == id) {
                    if(this.spec.spec_children[i].selectedItem == true) {
                        this.$set(this.spec.spec_children[i], "selectedItem", false);
                        this.activeItem = '';
                        if(this.activeItem == '') {
                            for(var ii in this.spec.spec_group) {
                                this.nospec = this.spec.spec_group[ii].name
                                break;
                            }
                            if(this.nospec == '') {
                                this.$emit('sendValue', null, null, null);
                        ...
                        this.activeItem = this.spec.spec_children[i].item;
                        this.selected = detail;
                        this.detailSubId = this.subIndex[index];
                        this.detailId = id

                        this.$emit('sendValue', detail, id, this.subIndex[index]);
                        this.arrayKey = id;
                        for(let ii = 0; ii < this.spec.spec_children.length; ii++) {
                            this.$set(this.spec.spec_children[ii], "selectedItem", false);
                        }
                        this.$set(this.spec.spec_children[i], "selectedItem", true);
                    }
                }
            }
    ...
    addClassTrue(index, event, id, groupid) {
        this.addClassClick = true
        this.activeItem = '';
        this.onecount = 0
    ...
    addClassEdit(index, event, id, groupid) {
        this.addClassClick = true
        this.activeItem = '';
        this.onecount = 0
                ...
                if(this.spec.spec_children[i].id == id) {
                  if(this.spec.spec_children[i].selectedItem && this.spec.spec_children[i].selectedItem == true) {
                      ...
                      // break;
                        ...
                        this.activeItem = this.spec.spec_children[i].item;
                        for(let ii = 0; ii < this.spec.spec_children.length; ii++) {
                            if(this.spec.spec_children[ii].spec_id == groupid) {
                                this.$set(this.spec.spec_children[ii], "selectedItem", false);
                            }
                        }
                        this.$set(this.spec.spec_children[i], "selectedItem", true);
        ...
        setTimeout(function() {
          ...
          if (goodsId) {
            self.$store.state.goods_id = goodsId;
            self.getData(goodsId, 'temp');
            self.specKey = goodsId;
          } else if(self.tempequal == false && Object.keys(selectedList).length != self.spec.spec_children.length) {
              self.getData(self.$route.params.goods_id);
          } else if(self.tempequal != false) {
              var count = 0;
              self.subIndex.forEach((item, index) => {
    ...
    checkStock(index, id) {
      ...
      if (count == count1 - 1) {
        for (var i in goods) {
          ...
          if (count3 == data.length && goods[i].goods_count == 0) {
    ...
    checkStock1() {
      ...
      data.forEach((item, index) => {
        for (var i in goods) {
          if (goods[i].key.indexOf(item) != -1 && goods[i].goods_count == 0) {
            var array = 0;
            var array1 = [];
            array1.push(item);
    ...
    toExchange() {
      ...
      this.$router.push({
    	...
    	toCard() {
				this.$router.push({
					...
					params: {
						good_id: this.$store.state.commodity_data.goods.id,
						num:this.$store.state.commodity_val,
						id:3
					},
				})
          ...
          this.axios
            .post(
              ...
              qs.stringify({
                ...
                price_new: this.data.goods.money,
                token: sessionStorage.getItem("data_token")
              })
            )
            .then(res => {
      ...
      buyNow(){
          ...
          this.$router.push({
              name:'order',
              params:{good_id: this.data.goods.goods_id, num: this.Number}
          })
