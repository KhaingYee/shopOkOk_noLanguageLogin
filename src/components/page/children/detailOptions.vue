...
<!-- data.goods.title -->
            ...
            <span v-else class='item' style='color:red;font-size:14px'>
              ...
              <em v-else-if="$store.state.commodity_data.goods.money">{{$store.state.commodity_data.goods.money}}</em>
    ...
    guige() {
      ...
      this.axios
        .post(
          ...
          qs.stringify({
            id: this.data.goods.goods_id?this.data.goods.goods_id:this.$route.params.id,
            token: sessionStorage.getItem("data_token")
          })
        )
        .then(res => {
