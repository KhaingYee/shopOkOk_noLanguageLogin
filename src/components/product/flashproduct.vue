    ...
    <div class="product">  
        ...
        <div class="header" :style="headBackgroundColor">
            ...
            <!-- <div :style="tabOpacity" class="tab">
              ...
              <div @click="handTab(3)" :class="{active:currentTab == 3}">推荐</div>
            </div> -->
            ...
            <transition  name="fade">
              ...
              <div class="bomb-box">
                ...
                <!-- <div class="btn" @click="toLink('/Cart')">
                <span class="icon"><i></i></span>购物
                </div> -->
              </div>
              </div>
            </transition>
        ...
        <div id="tab1">
            <!-- <div @focusout="handleFocusOut1" @focus="handleFocus1" tabindex="0">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit. Veritatis, libero provident? Architecto iure labore itaque provident tenetur sint hic velit aspernatur. Esse ipsum maiores voluptatibus nam eius consequuntur voluptatem minus!
            </div> -->
            <PullUpReload
            v-show="tabFlg"
            ...
            >
                    ...
                    <mt-swipe :auto="4000" @change="imgSlideChange">
                      ...
                      <mt-swipe-item v-for="(item,index) in images" :key="item.id">
                                            
                      <img :src="URL + item.pic_url" />
                      </mt-swipe-item>
                    </mt-swipe>
                    <!-- <div class="see-video" v-for="(videoitem,index) in video" :key="index" v-if="videoitem.goods_video">
                      <img src="../../assets/videoplay.png" class="playimg" @click="showvideo(videoitem.goods_video)" v-if="videoitem.goods_video"/>
                    </div> -->
                    ...
                    <div class="FlashDetail">
                        <div class="title"></div>
                        <div class="count">
                            <div class="time">剩余时间 &nbsp;{{showTimeH}}:{{showTimeM}}:{{showTimes}} </div>
                            <div class="Fsale-sold">
                                <div class="sold-num">{{productData.buy_num}}&nbsp;sold</div>
                                <div class="sold-pho">
                                    <div class="sold-show" :style="'width:' + ((productData.buy_num/productData.activity_num)*100) + '%'"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="describe" v-if="$store.state.commodity_data">
                      <div>
                          ...
                          <!-- <div v-if="opt"> -->
                          <p class="fn" v-if="detail">{{$store.state.commodity_data.list.title}}</p>
                          <!-- <p class="fn" v-else>{{$store.state.commodity_data.goods.title}}</p> -->
                          <p class="fn" v-else>{{productData.title}}</p>

                          <p class="description-fn" v-if="detail">{{$store.state.commodity_data.list.description}}</p>
                      ...
                      <p class="price" v-if="$store.state.commodity_data.list.p_id==0">
                        <span style="color:red;font-size:.36rem">
                        ￥
                        {{productData.activity_price}}
                        </span>
                        <span class="new">
                        原价 ：￥
                        ...
                        <span class="report" @click="handleNotice(productData, images)">举报</span>
                      </p>
                      <p class="price" v-else>
                        <span style="color:red;font-size:.36rem">
                        ￥
                        {{$store.state.commodity_data.list.activity_price}}
                        </span>
                        <span class="new">
                        原价 ：￥
                        <s>{{$store.state.commodity_data.list.price_market}}</s>
                        </span>
                        <!--        <span class="report" @click="Report">举报</span>-->
                        <span class="report" @click="handleNotice(productData, images)">举报</span>
                      </p>
                    ...
                    <div class="selected" @click="theSon">
                        ...
                        <div v-if="!opt">-->
                        <div v-for="(hh,index) in specificationNew" :key="index">
                            ...
                            <span class="select-spec" v-if="newspec == hh.id">{{hh.item}} {{$store.state.commodity_val}}个</span>
                        </div>
                    ...
                    <!-- <div class="for-discount" @click="showCoupon" v-if="couponList.length != 0">
                    ...
                    <div class="delivery_method">
                      ...
                      <div v-if="productData.freight_type == 2">
                        <div>
                          <div class="delivery_title">运费</div>
                            ...
                            <div class="pro_title" v-if="gift && Array.isArray(gift) == false">促销</div>
                                <div v-for="(item,index) in gift.goods" :key="index" v-if="gift.length != 0">
                                    ...
                                    <div style="margin: 0rem .2rem .1rem .3rem;">
                                        ...
                                        <!-- <span v-else>sss-ss</span> -->
                                    </div>
                                </div>
                                <!-- <div class="showpormotion" v-for="(item,index) in gift" :key="index" v-if="gift.length != 0">
                                  <img :src="URL + item.pic_url" class="showpormotion-img" v-if="item.pic_url !=null">
                                  <div class="showpormotion-all">
                                    <div class="showpormotion-title">{{item.title}}</div>
                                            <div class="showpormotion-num" v-if="item.give_num != 0 && item.give_num != null">x{{item.give_num}}</div>
                                  </div>
                                </div> -->
                                <div class="promotion-card" v-if="reduction">
        ...
        <div id="tab2" >
            <DropDownRefresh :on-refresh="onRefresh">
                <detailspage></detailspage>
            </DropDownRefresh>
        </div>
        
        <div id="tab3" >
            <DropDownRefresh1 :on-refresh="onRefresh1">
        ...
        <foot-btn
            ...
            :footData="detail !== '' ? detArr : optArr"
            :speciLen="lengthSpeci"
            :shopfb="shopData.id"
            :promotion="promotion"
        ></foot-btn>
...
import FootBtn from "@/components/product/childd/footBtn.vue"; // 底部按钮
import PageOption from "@/components/page/children/pageOption.vue";
import shopInfor from "@/components/product/childd/shopInfor.vue"; // 店铺信息
import detailOption from "@/components/product/childd/detailOptions.vue"; // 立即购买 || 加入购物车 弹框
...
import $ from "jquery";
import comboList from "@/components/page/children/listCombo.vue";
...
export default {
  ...
  data() {
    return {
      ...
      specificationNew:'',
      headParams: {
        title: sessionStorage.getItem('titlepro'),
        description: sessionStorage.getItem('descriptionpro'),
        keywords: sessionStorage.getItem('keypro'),        
      },
      nospec: '',
      lengthSpeci: 0,
      comboList: [],
  ...
  mounted() {
      var self = this
        window.setTimeout(function () {
            self.headParams.title = sessionStorage.titlepro
            self.headParams.description = sessionStorage.descriptionpro
            self.headParams.keywords = sessionStorage.keypro
  ...
  methods: {
      ...
      scrollTrue() {
          window.onscroll = () => {
            let firTab = document.querySelector('.firstTab')
            if (firTab && this.isScrolledIntoView(firTab)) {
              this.currentTab = 0;
            }

            let secTab = document.querySelector('.secondTab')
            if (secTab && this.isScrolledIntoView(secTab)) {
              this.currentTab = 1;
            }

            let thiTab = document.querySelector('.thirdTab')
            if (thiTab && this.isScrolledIntoView(thiTab)) {
              this.currentTab = 2;
            }

            let fourTab = document.querySelector('.fourthTab')
            if (fourTab && this.isScrolledIntoView(fourTab)) {
              this.currentTab = 3;
            }
          }
      },
      entryGoods(id) {
          window.open(window.location.origin + "/product/" + id + "/1");
      },
      handleFocus1() {
      },
      defaultSpec() {
          ...
          .then(res => {
              if (res.data.status == 1) {
                  ...
                  this.specificationNew = res.data.data.spec.spec_children;
      ...
      apicall() {
        this.axios.get(this.$httpConfig.wechatShareAPI, qs.stringify({
          token: sessionStorage.getItem("data_token")
        }))
          .then(res => {
    ...
    // },
    getDeliveryCal(){
        let choiceId = this.$store.state.goods_id ? this.$store.state.goods_id : this.$route.params.id;
        this.axios.post(this.$httpConfig.deliveryCalcus,qs.stringify({
					store_id: this.$store.state.commodity_data.list.store_id,
    ...
    AArrName() {
      ...
      handleNotice(title, img) {
          sessionStorage.setItem('productInform', JSON.stringify({title, img}))
    ...
    theSon() {
      ...
      this.axios({
        url: this.$httpConfig.seckillDetail,
        method: "get",
        params: {
          goods_id: this.$store.state.commodity_data.list.goods_id, token: sessionStorage.getItem("data_token")
        }
      })
        .then(res => {
        ...
        //   this.video = res.data.data.video;
          ...
          this.store__id = res.data.data.list.store_id;
          this.$store.state.commodity_data = res.data.data;
          sessionStorage.setItem("descriptionpro",res.data.data.list.description);
          sessionStorage.setItem("keypro",res.data.data.list.title);
        })
        .catch(err => {
          console.log(err);
        });
    ...
    // spec() {},
      ...
      shopInfo() {
        this.axios({
          ...
          method: "get",
          params: {
            id: this.$store.state.commodity_data.list.store_id,
            token: sessionStorage.getItem("data_token")
          }
        })
                .then(res => {
                  this.shopData = res.data.data;
                  sessionStorage.setItem("titlepro",res.data.data.shop_name);
                })
                .catch(err => {
                  console.log(err);
                });
      ...
      ax() {
          this.axios({
              ...
              this.p_id = res.data.data.list.p_id;
              this.store__id = res.data.data.list.store_id;
              this.route_id = res.data.data.list.id;
    ...
    sendValue(detail, id, subid, optArr){
        this.gift = this.$store.state.commodity_data.promotion.gift
        this.reduction = this.$store.state.commodity_data.promotion.reduction;
        //获取套餐
        this.getPackageSub();
        if(detail == null) {
            this.option = ''
    ...
    getPackageSub(){
      this.axios.post(
            this.$httpConfig.matchGood,
            qs.stringify({
                goods_id: this.$store.state.commodity_data.list.id,
                token: sessionStorage.getItem("data_token")
            })
        )
        .then(res => {
  ...
  <style lang="less" scoped>
      ...
      .FlashDetail{
          height: .65rem;
          padding: .2rem;
          background: url(../../assets/Flah.png) no-repeat;
          background-size: 100% 100%;
          .title{
              float: left;
              font-size: .26rem;
...
https://renatello.com/vue-js-scroll/
-->
