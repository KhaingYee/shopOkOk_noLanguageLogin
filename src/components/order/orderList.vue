<template>
    <div ref="order_list">
        <!-- <div v-title :data-title="$store.state.order_title" v-if="$store.state.order_title">
            {{$store.state.order_title}}
        </div> -->
        ...
        <div class="nav-wrap">
            <ul class="nav">
                <li class="fl" v-for="(name,index) in datas.name" :key="name.id" :class="{active:isactive == index}"
                    @click="addClass(index,'click')"><span>{{name}}</span></li>
            </ul>
        </div>
            ...
            <div class="order-list" v-for="(item,index) in order" :key="item.id">
                ...
                <div class="describe clearfix" v-for="text in item.goods" :key="text.id">
                    ...
                    <div class="fl words" @click="toDetails(item,text,index)">
                        ...
                        <span v-if="text.goods_price !== 0" style="padding: 0 .2rem;" class="fl price">￥{{text.goods_price}}</span>
                        <!-- <span v-if="text.goods_price !== '0.00'" style="padding: 0 .2rem;" class="fr number">x{{text.goods_num}}</span> -->
                        <span v-else style="padding: 0 .2rem;">x1 赠品</span>
                        <br>
                        <span v-if="text.goods_price !== '0.00'" style="padding: 0 .2rem;" class="fl price">
                            <p v-if="(item.delivery == '1' || item.delivery == '2') && item.distribution_fee == '0.00'">
                                免配送费
                            </p>
                            <p v-else-if="(item.delivery == '1' || item.delivery == '2') && item.distribution_fee !== '0.00'">
                                含配送费 {{item.distribution_fee}} 元
                            </p>
                            <p v-else-if="item.delivery == '0' && item.shipping_monery == '0.00'">
                                包邮
                            </p>
                            <p v-else-if="item.delivery == '0' && item.shipping_monery !== '0.00'">
                                含运费 {{item.shipping_monery}} 元
                            </p>
                        </span>
                    </div>
                    <button class="btn" @click="toRouter('/evaluate',item,text)" v-if="item.order_state == 1">马上评论
                    </button>
                </div>
                <div class="commod clearfix">
                    <span class="fl total">共{{item.count}}件商品</span>
                    <span class="fl actual">实付款&nbsp;:&nbsp;<span>￥{{item.price_sum}}</span></span>
                    <span class="icon" v-if="item.order_status == 4 || item.order_status == -1"
                          @click="deletOrder(item,index)"></span>
                </div>
                <div class="operation clearfix">
                    ...
                    <button class="btn fr" @click="toRouter('/pay',item,false)" v-if="item.order_status == 0">马上付款
                    </button>
                    <button class="btn fr" @click="toRouter('/order',item,false)" v-if="item.order_status == 1">查看订单
                    </button>
                    <button class="btn fr" @click="toRouter('/logis',item,false)" v-if="item.order_status == 3">查看物流
                    </button>
                    <button class="butn fr" @click="take(item,false)" v-if="item.order_status == 3">确认收货</button>
                    <button class="btn fr repeat" @click="toRouter('/Cart',item,1)"
                            v-if="item.order_status == 4 || item.order_status == -1" v-show="item.order_state != '1'">
                        再次购买
                    </button>
                    <button class="btn fr repeat" @click="toRouter('/evaluate',item,1)"
