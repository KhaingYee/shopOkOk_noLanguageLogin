	...
	<div>
				...
				<li class="clearfix" v-for="(item,index) in order.goods" :key="index">
					...
					<div class="explain-wrap fr">
						...
						<p class="clearfix product" @click="enterShop(index,item)">
							<span v-if="item.goods_price !== 0.00" class="fl price">￥{{item.goods_price}}</span>
							<span v-if="item.goods_price !== 0.00" class="fr number">x{{item.goods_num}}</span>
							<span v-else>x1 赠品</span>
						</p>
		...
		<div class="sundry-wrap">
			<p class="mode clearfix" v-if="order.order_status !=0">
				...
				<span class="answer fr">{{order.type_name}}</span>
			</p>
			<p class="mode clearfix" v-if="order.order_status !=0">
				<span class="title fl">付款时间：</span>
				<span class="answer fr">{{getPayTime}}</span>
			</p>
			<p class="mode clearfix" v-if="$route.params.order_type == 0 ||$route.params.order_type == 1">
				<span class="title fl">所扣除积分</span>
				<span class="answer fr">{{order.interagl_total}}</span>
			</p>
			...
			<div class="money-wrap" v-if="$route.params.order_type == 2">
				<p class="clearfix">
					...
					<span class="fr name" style="color: #d0111b;">￥{{parseFloat(goodsPrice)}}</span>
				</p>
				<!-- <p class="clearfix">
				...
				<!-- ***** -->
				<p class="clearfix" v-if="deli && deli == '1'">
					<span v-if="distFee == '0.00'" class="fl name">配送共计</span>
					<span v-else class="fl name">配送共计</span>

					<span v-if="distFee == '0.00'" class="fr price">免配送费</span>
					<span v-else class="fr price">{{distFee}}</span>
				</p>
				<p class="clearfix" v-if="deli && deli == '2'">
					<span v-if="distFee == '0.00'" class="fl name">配送共计</span>
					<span v-else class="fl name">配送共计</span>
				...
				<p class="clearfix" v-if="deli== '0' || deli == '0.00'">
					<span v-if="shipMonery == '0.00'" class="fl name">运费共计</span>
					<span v-else class="fl name">运费共计</span>

					<span v-if="shipMonery == '0.00'" class="fr price">包邮</span>
					<span v-else class="fr price">{{shipMonery}}</span>
				</p>
				<!-- ***** -->
				<p class="clearfix" v-if="order.coupon_deductible !== '0.00'">
					<span class="fl name">优惠券</span>
					<span class="fr price">- ￥{{order.coupon_deductible}}</span>
				</p>
				<p class="clearfix" v-if="cutvalue">
					<span class="fl name">满减</span>
					<span class="fr price">-{{cutvalue}}元</span>
				</p>
				<p class="clearfix">
					<span class="fl name">实付款</span>
					<span class="fr price">￥{{order.price_sum}}</span>
				</p>
			</div>
				
			<!-- <div class="Integral-Mode" v-if="$route.params.order_type == 1">
				<p class="clearfix" v-if="deli && deli == '1'">
					<span v-if="distFee == '0.00'" class="fl name">配送共计</span>
					<span v-else class="fl name">配送共计</span>
			...
			<div class="money-wrap" v-if="$route.params.order_type == 1">
				<p class="clearfix">
					<span class="fl name">额外支付金额</span>
					<!-- <span class="fr price">￥{{parseFloat(order.price_sum)}}</span> -->
					<span class="fr name" style="color: #d0111b;">￥{{goodsPrice}}</span>
				</p>
				<!-- <p class="clearfix">
					<span class="fl name">运费</span>
					<span class="fr price">+ <em>￥{{order.shipping_monery}}</em></span>
				...
				<!-- ***** -->
				<p class="clearfix" v-if="order.coupon_deductible ">
					<span class="fl name">优惠券</span>
					<span class="fr price">- ￥{{order.coupon_deductible}}</span>
				</p>
				<p class="clearfix" v-if="cutvalue">
...
<script>
	...
	export default {
		...
		created(){
			let title = "订单详情" + '-' + sessionStorage.getItem('titleKey') + '-' + sessionStorage.getItem('updateDescription');
		...
		methods: {
			...
			toService(item, id,type) {
				if(this.deli == 1 && this.distFee !==0.00) {
				var	totalMoney = Number(this.order.price_sum) + Number(this.distFee);
				}
				else if(this.deli == 2 && this.distFee !==0.00) {
				var	totalMoney = Number(this.order.price_sum) + Number(this.distFee);
				}
				else if(this.deli == 0 && this.shipMonery !==0.00) {
				var	totalMoney = Number(this.order.price_sum) + Number(this.shipMonery);
				}
				else if(this.deli == 0.00 && this.shipMonery !==0.00) {
				var	totalMoney = Number(this.order.price_sum) + Number(this.shipMonery);
				}
				else {
				var	totalMoney = Number(this.order.price_sum)
				}
				...
				if(this.$route.params.order_status) {
					...
					this.$router.push({
						...
						params: {
							status: 3,
							id: this.order.id,
							order_status:this.$route.params.status
						}
					});
				}
			},
			...
			// integralPay(){
			// 	// if(this.deli == 1 && this.distFee !==0.00) {
			// 	// var	total = Number(this.IntegralPri) + Number(this.distFee);
			// 	// }
			// 	// else if(this.deli == 2 && this.distFee !==0.00) {
			// 	// var	total = Number(this.IntegralPri) + Number(this.distFee);
			// 	// }
			// 	// else if(this.deli == 0 && this.shipMonery !==0.00) {
			// 	// var	total = Number(this.IntegralPri) + Number(this.shipMonery);
			// 	// }
			// 	// else if(this.deli == 0.00 && this.shipMonery !==0.00) {
			// 	// var	total = Number(this.IntegralPri) + Number(this.shipMonery);
			// 	// }
			// 	// else {
			// 	// var total = Number(this.IntegralPri)
			// 	// }
			// 	if(this.address == '') {
			// 		Toast({
			// 			message: '请先填写地址'
			...
			integralPay(){
				...
				sessionStorage.setItem('pay_sourse','integral')
					this.$router.push({
						...
						params: {
							id: 1, 
							integral:this.order.interagl_total,
							money:total,
						},
						query:{
							goods:this.$route.query.goods,
							orderId:this.order.order_sn_id
						}
					});
			},
			pay(item) { //马上付款
				...
				if(this.deli == 1 && this.distFee !==0.00) {
				var	total = Number(item.price_sum) + Number(this.distFee);
				}
				else if(this.deli == 2 && this.distFee !==0.00) {
				var	total = Number(item.price_sum) + Number(this.distFee);
				}
				else if(this.deli == 0 && this.shipMonery !==0.00) {
				var	total = Number(item.price_sum) + Number(this.shipMonery);
				}
				else if(this.deli == 0.00 && this.shipMonery !==0.00) {
				var	total = Number(item.price_sum) + Number(this.shipMonery);
				}
				else {
				var total = Number(item.price_sum)
				}
				if(this.$route.params.order_type == 1) {
					this.integralPay();
				} else {
			...
			if(this.$route.params.order_type == 1) {
				this.axios.post(this.$httpConfig.integralGoodsInfo, qs.stringify({
					...
					if(res.data.status == 10001) {
						...
						this.IntegralPri = res.data.data.price_sum;
						this.distFee = this.order.distribution_fee;
						this.deli = this.order.delivery;
						this.shipMonery = this.order.shipping_monery;
						this.load_wrap = false;
						this.address=res.data.data;
						this.goodsPrice = this.order.money;
					}
				...
				this.axios.post(this.$httpConfig.orderDetail, qs.stringify({
					...
					if(res.data.status == 10001) {
						...
						this.provId = res.data.data.address.prov_id;
          				this.cityId = res.data.data.address.city_id;
         				this.areaId = res.data.data.address.dist_id;
						this.distFee = this.order.distribution_fee;
						this.deli = this.order.delivery;
						this.shipMonery = this.order.shipping_monery;
						this.cutvalue = this.order.goods[0].cut;
						this.goodsPrice = this.order.goods[0].goods_price;
		...
		computed:{
			getPayTime(){
				var date = new Date(parseInt(this.order.pay_time)*1000);
