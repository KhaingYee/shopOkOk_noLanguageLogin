<template>
	<div class="box">
		...
		<div class="head">
			<ul :style="{width:titleData.length * 4+'rem'}" class="headers f2" ref="headers">
				<li :class="{avtive:titleAddIndex == index}" class="f2" @click="proAddTab(index,item.id)" v-for="(item, index)  in address" :key="index">
					{{item.name}}
				</li>
			</ul>
		</div>
		<div class="head">
			<ul :style="{width:titleData.length * 2+'rem'}" class="headers fl" ref="headers">
				<li :class="{avtive:titleIndex == index1}" class="fl" @click="fixproTab(index1)">
					热门店铺
				</li>
		...
		methods: {
			...
			storeClasses() {
				this.axios.post(this.$httpConfig.storeClasses, qs.stringify({
					...
					.then((res) => {
						this.titleData = res.data.data.class;
