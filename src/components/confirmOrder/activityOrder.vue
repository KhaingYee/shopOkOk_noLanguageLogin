    ...
    <div class="order-wrap">
            ...
            <li class="clearfix" id="store">
                ...
                <div class="pull-right fl">
                    ...
                    <p class="price-wrap clearfix">
                        ...
                        <span class="number fr">X{{number}}</span>
                    </p>
                </div>
            </li>
            <li>
                <div class="dist-wrap">
                    ...
                    <!-- <div class="hd clearfix" @click="selectCoupon(goods_data.store_id, 0)" v-if="$route.params.id != 3" v-show="$route.params.status !=5">
                    ...
                    <div class="dd">
                        ...
                        <textarea placeholder="选填：备注限字在45个字以内" oninput="if(value.length>45)value = value.slice(0,45)"
                                  v-model="message"></textarea>
                        <p class="ind">共 <span>{{number}}</span> 件商品</p>
                    </div>
                </div>
            </li>
        </ul>
        <div class="price-set-wrap">
            <div class="total price clearfix">
                ...
                <span class="fr">￥{{goods_data.activity_price * number}}</span>
            </div>
            ...
            <div class="footer clearfix">
                ...
                <div class="fr money">
                    实付款&nbsp;:&nbsp;<span class="price">￥
						<span v-if="money == -1">{{(Number(goods_data.activity_price) * Number(number) * (Number(cartDis.discount)/100) - Number($store.state.const_coupon_price))|keepTwoNum}}</span>
                        <span v-else-if="money">{{(Number(goods_data.activity_price) * Number(number) * (Number(cartDis.discount)/100) + Number(money) - Number($store.state.const_coupon_price))|keepTwoNum}}</span>
                        <span v-else>{{(Number(goods_data.activity_price) * Number(number) * (Number(cartDis.discount)/100) + Number(freight) - Number($store.state.const_coupon_price))|keepTwoNum}}</span>
					</span>
                </div>
            </div>
        </div>
        ...
        methods: {
            ...
            getconfirmOrder(){
                this.axios.post(this.$httpConfig.requeryConfirmOrderId,qs.stringify({
                    ...
                    token: sessionStorage.getItem("data_token"),
                    goods_id: this.$route.params.goods_id}),'post').then((res) => {
            ...
            getData() {
                if (this.$route.params.stat == 1 || this.$route.params.stat == 2) {
                    this.axios.post(this.$httpConfig.panicCartGoodsDetail, qs.stringify({
                ...
                } else if (this.$route.params.stat == 4) {
                    this.$HTTP(this.$httpConfig.bargainGoodsInfo, {
                        id: this.$route.params.id,
            ...
            toCashierAll() {
                ...
                if(this.money == -1){
                    var total = parseFloat(this.goods_data.activity_price * (this.number) * Number(this.cartDis.discount)/100) - parseFloat(this.$store.state.const_coupon_price);
                }
                else if(this.money){
                    var total = parseFloat(this.goods_data.activity_price * (this.number) * Number(this.cartDis.discount)/100) + parseFloat(this.money) - parseFloat(this.$store.state.const_coupon_price);
                }
                else{
                    var total = parseFloat(this.goods_data.activity_price * (this.number) * Number(this.cartDis.discount)/100) + parseFloat(this.freight) - parseFloat(this.$store.state.const_coupon_price);
                }
                ...
                if (this.$route.params.stat == 1) {
                    if(this.$route.query.order_id) {
                        var data = {
                            ...
                            prom_id: this.goods_data.prom_id,
                    ...
                    this.axios.post(this.$httpConfig.generateOrder, qs.stringify(data
                        /* id:price_member,*/
                        //收货地址ID
                        ...
                        this.clearData();
                        sessionStorage.removeItem('icanvoiceGroup');
                        sessionStorage.removeItem('invoiceInit');
                        sessionStorage.setItem('AssembleOrderId', JSON.stringify({
                            id: this.$route.params.Successid
                        ...
                        // if (res.data.status == 1 && res.data.data.id !== undefined) {
                        //     sessionStorage.setItem('pay_sourse', 'goods');
                        ...
                        //         }
                        //     });
                        // } else if (res.data.status == 1 && res.data.data.order_id !== undefined) {
                        //     sessionStorage.setItem('pay_sourse', 'goods');
                        //     let message = res.data.data.message;
                        //     Toast({
                        //         message: message,
                ...
                else if (this.$route.params.stat == 2) {  // 去参团
                    if(this.$route.query.order_id) {
                        var data = {
                        token: sessionStorage.getItem("data_token"),
                        message: this.message, //留言
                ...
                else if (this.$route.params.stat == 3) {  //秒杀提交生成订单
                    this.axios.post(this.$httpConfig.orderBeginFromGroupPurchase, qs.stringify({
                        /* id:price_member,*/
                        //收货地址ID
                        token: sessionStorage.getItem("data_token"),
                        message: this.message, //留言
            ...
            tolink(to, id) {
                ...
                sessionStorage.removeItem('pickup_info');
                sessionStorage.removeItem('goods_data');
                sessionStorage.setItem('active_goods_data', JSON.stringify({
                        // id: this.$route.params.id,
