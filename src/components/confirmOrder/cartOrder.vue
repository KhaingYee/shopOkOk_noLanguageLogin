	...
	<div class="order-wrap">
		...
		<div class="price-set-wrap"  >
			<div class="total price clearfix">
				...
				<!-- <span class="fr" v-if="status == 1">￥{{parseFloat(total_price)+parseFloat(money)}}</span> 
				<span class="fr" v-else>￥{{total_price}}</span> -->
				<span class="fr">￥{{parseFloat(total_price)}}</span>
			</div>
			<!-- <div class="freight price clearfix" v-for="(item, index) in store" :key="index">
				<div v-if="status == 1">
					<span class="fl">配送 ( {{item.shop_name}} )</span>
					<span class="fr">+&nbsp;&nbsp;<i>￥{{parseFloat(money)}}</i></span>
				</div>
				<div class="freight price clearfix" v-else>
					<span class="fl">运费 ( {{item.shop_name}} )</span>
					<span class="fr">+&nbsp;&nbsp;<i>￥{{parseFloat(freight)}}</i></span>
				</div>
			</div> -->
			<div class="freight price clearfix" v-for="(item, index) in store" :key="index">
				<div v-if="status == 1">
					<span class="fl">配送 ( {{item.shop_name}} )</span>
					<span v-if="money == null" class="fr">+&nbsp;&nbsp;<i>￥0</i></span>
					<span v-else class="fr">+&nbsp;&nbsp;<i>￥{{parseFloat(money)}}</i></span>
				</div>
				<div class="freight price clearfix" v-else>
					<span class="fl">运费 ( {{item.shop_name}} )</span>
					<span v-if="freight == null" class="fr">+&nbsp;&nbsp;<i>￥0</i></span>
					<span v-else class="fr">+&nbsp;&nbsp;<i>￥{{parseFloat(freight)}}</i></span>
				</div>
			</div>
			...
			<div class="footer clearfix">
				...
				<div class="fr money">
					实付款&nbsp;:&nbsp;<span class="price">￥
						...
						<span v-else>{{all_price|keepTwoNum}}</span> -->
						<span v-if="status == 1">
						<span v-if="storeIID == 3">{{parseFloat(total_price)+2*parseFloat(money)-parseFloat($store.state.const_coupon_price)|keepTwoNum}}</span>
						<span v-else-if="money == null">{{parseFloat(total_price)-parseFloat($store.state.const_coupon_price)|keepTwoNum}}</span>
						<span v-else>{{parseFloat(total_price)+parseFloat(money)-parseFloat($store.state.const_coupon_price)|keepTwoNum}}</span>
						</span>
	
						<span v-else>
						<span v-if="storeIID == 3">{{parseFloat(total_price)+2*parseFloat(freight)-parseFloat($store.state.const_coupon_price)|keepTwoNum}}</span>
						<span v-else-if="freight == null">{{parseFloat(total_price)-parseFloat($store.state.const_coupon_price)|keepTwoNum}}</span>
						<span v-else>{{parseFloat(total_price)+parseFloat(freight)-parseFloat($store.state.const_coupon_price)|keepTwoNum}}</span>
						</span>
					</span>
				</div>
			</div>
	...
	export default {
		...
		// 提交订单
			toCashierAll() {
				...
				if(this.money) {
					if(this.storeIID == 3) {
					var price = parseFloat(this.total_price) + 2*parseFloat(this.money) - parseFloat(this.$store.state.const_coupon_price);
					}
					else if(this.money == null) {
					var price = parseFloat(this.total_price) - parseFloat(this.$store.state.const_coupon_price);
					}
					else {
						var price = parseFloat(this.total_price) + parseFloat(this.money) - parseFloat(this.$store.state.const_coupon_price);
					}
				}
				else {
					if(this.storeIID == 3) {
						var price = parseFloat(this.total_price) + 2*parseFloat(this.freight) - parseFloat(this.$store.state.const_coupon_price);
					}
				...
				this.axios.post(this.$httpConfig.goBuy, qs.stringify({
					...
					total:parseFloat(lastPrice), //订单总价
