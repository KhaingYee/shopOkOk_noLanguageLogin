		...
		<ul class="mark-wrap">
			...
			<li>
				<div class="dist-wrap">
					...
					<div class="hd clearfix" @click="selectCoupon(store_id, 0)" v-if="$route.params.id != 3">
						<div class="title fl">选择优惠券</div>
					...
					<div class="hd clearfix" @click="tolink('/invoice',store_id)" >
						<div class="title fl">开具发票</div>
						<div class="busi fr" v-if="show[0] == 1">已开发票</div>
						<div class="busi fr" v-else>无需发票</div>
					</div>
					<div class="dd">
						...
						<textarea placeholder="选填：备注限字在45个字以内" oninput="if(value.length>45)value = value.slice(0,45)" v-model="remarks[store_id]"></textarea>
			...
			getData(){
                this.axios.post(this.$httpConfig.purchaseImmediately,qs.stringify({
					...
					if(res.data.status == 1){
						...
						this.store_id = this.order_package[0].store_id;
						this.remarks[this.store_id] = '';
						if(sessionStorage.getItem('invoiceGroup')){
							...
							let invoiceData = {};
							invoiceData[this.store_id] = {translate:0};
