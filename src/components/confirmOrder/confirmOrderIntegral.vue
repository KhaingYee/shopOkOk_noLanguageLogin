<template>
	<div class="order-wrap">
		...
		<!-- 单个商品 -->
		<ul class="mark-wrap">
			<li class="header"><img :src="URL + goods_data.shop_logo" alt="" /><span>{{goods_data.shop_name}}</span></li>
		...
		<Shopsn></Shopsn>
		<detail-coupon ref="coupon" :integral_id="goods_data.id"></detail-coupon>
	...
	export default {
		...
		// 提交订单 单个商品购买
			toCashierAll() {
				...
				if(this.money == -1){
					var total = parseFloat(this.goods_data.money * this.goods_data.goods_num * Number(this.cartDis.discount)/100);
				}
				else if(this.money){
					var total = parseFloat(this.goods_data.money * this.goods_data.goods_num * Number(this.cartDis.discount)/100) + parseFloat(this.money);
				}
				else{
					var total = parseFloat(this.goods_data.money * this.goods_data.goods_num * Number(this.cartDis.discount)/100) + parseFloat(this.freight);
				}
