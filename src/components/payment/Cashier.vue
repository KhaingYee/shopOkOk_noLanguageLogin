	...
	export default {
		...
		methods: {
			...
			callInitiate() {
				var that = this;
				let tempApi = null
				if(this.$route.params.activityOrder && this.$route.params.activityOrder === 1) {
					tempApi = this.$httpConfig.payOrderGroupInitiatePayment
				} else {
					tempApi = this.$httpConfig.initiatePayment
				}
				this.axios.post(tempApi, qs.stringify({
