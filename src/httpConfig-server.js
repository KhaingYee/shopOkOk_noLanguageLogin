// import logs from 'D:/TDA Project/data.js' // Local
import logs from '../center.zjlmapp.com/config.ini' // Server

load(success);
var xiexie;
function load(success) {
    var thanksYou;
    // var url = 'http://localhost:8087/config.ini'; //A local page
    // var url = 'http://106.75.61.4/config.ini'; //A local page
    var url = 'http://m.shopsn.net/config.ini'; //A local page
    var xhr = new XMLHttpRequest();
    xhr.open('GET', url, false);
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            thanksYou = JSON.parse(xhr.responseText)
            success(thanksYou);
        }
    }
    xhr.send();
...
// console.log('xiexie ' + JSON.stringify(xiexie.APIName))
...
const request = xiexie.APIName[0].request; // thinkphp version 6.0

const imgRequest = xiexie.APIName[0].imgRequest;
...
const userModuleIndex = xiexie.APIName[0].userModuleIndex; // thinkphp version 6.0
const uploadImage = 'upload.php/';

const config = {
    fileReaderCode: xiexie,
    ...
    matchGood: request + 'GoodsCombo/packageSub', // 最佳搭配套餐
    ...
    CompanyStoreAduitProgress: userModuleIndex + 'CompanyStoreAduitProgress/getStoreByUser', //企业入驻进度查询
    ...
    PersonStoreAduitProgress: userModuleIndex + 'PersonStoreAduitProgress/getStoreByUser', //个人入驻进度查询
